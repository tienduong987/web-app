<template>
  <div :id="id" class="list-product">
    ...
    <swiper v-if="listProducts.length > 3" class="swiper" :options="swiperOption" ref="swiperRef">
      <swiper-slide class="carousel-item cursor" v-for="item in listProducts" :key="item.id">
        ...
        <!-- <div class="favourite cursor">
          <base-icon icon="icon-heart" class="d-iflex" size="32" />
        </div> -->
      </swiper-slide>
    ...
    <div v-if="listProducts.length > 3" class="btn-crousel cursor prev" @click="handleClickArrow('prev')">
      <base-icon icon="icon-prev" size="32" class="d-iflex" />
    </div>
    <div v-if="listProducts.length > 3" class="btn-crousel cursor next" @click="handleClickArrow('next')">
...
<script lang="ts">
  ...
  export default class DetailListProduct extends Vue {
    ...
    handleClick(id: number): void {
      const _id = id + ''
      this.$router.push({ name: 'DetailProject', params: { type: 'product', id: _id } })
    }
