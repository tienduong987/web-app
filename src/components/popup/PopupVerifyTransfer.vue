  ...
  export default class PopupVerifyTransfer extends Mixins(PopupMixin) {
    ...
    async submit(): Promise<void> {
      ...
      if (this.typeAction && this.typeAction === 'TRANSFER') {
        await apiLyn
          .transferLyn(this.typeActive.toLowerCase(), { ...this.data, verificationCode: this.form.code })
          .then(() => {
            this.success = 1
        ...
        await apiLyn
          .withdrawLyn(this.currency.toLowerCase(), { ...this.data, verificationCode: this.form.code })
          .then(() => {
            this.success = 1
            this.isLoading = false
